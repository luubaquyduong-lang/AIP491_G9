...
# 🗺️ Chuyên mục cần crawl (VD: Phan Thiết)
base_url = "https://www.ivivu.com/blog/category/viet-nam/tp-hcm/"
headers = {"User-Agent": "Mozilla/5.0"}
data = []

START_PAGE = 1   # 👈 Trang bắt đầu
MAX_PAGE = 999   # 👈 Crawl đến khi hết bài
csv_file = "ivivu_tp_hcm.csv"

# 🟢 Kiểm tra file có sẵn không
file_exists = os.path.exists(csv_file)

# 🟢 Crawl phần GIỚI THIỆU chuyên mục (extra-cate-content)
print(f"\n📘 Đang tải phần giới thiệu của chuyên mục: {base_url}")
try:
    main_resp = requests.get(base_url, headers=headers, timeout=15)
    if main_resp.status_code == 200:
        main_soup = BeautifulSoup(main_resp.text, "html.parser")
        intro_div = main_soup.find("div", class_="extra-cate-content")
        if intro_div:
            intro_text = " ".join(p.get_text(strip=True) for p in intro_div.find_all("p"))
            data.append({
                "title": "Giới thiệu về chuyên mục TP.HCM",
                "link": base_url,
                "image_url": "",
                "date": "",
                "views": "",
            ...
            print("✅ Đã lấy phần giới thiệu chuyên mục.")
        else:
            print("⚠️ Không tìm thấy phần giới thiệu chuyên mục.")
    else:
        print(f"⚠️ Lỗi {main_resp.status_code} khi tải trang chính.")
except Exception as e:
    print(f"❌ Lỗi khi tải phần giới thiệu: {e}")

# 🟣 Crawl danh sách bài viết trong chuyên mục
for page in range(START_PAGE, MAX_PAGE + 1):
    ...
    for article in articles:
        ...
        # 🟡 Crawl nội dung chi tiết của bài
        try:
...
# ✍️ Lưu vào CSV (append nếu có sẵn)
...
print(f"\n🎉 Đã thêm {len(data)} bài mới (bao gồm phần giới thiệu) vào file {csv_file}")
